...
- 将对话条拖出后再展开历史，或先展开历史再拖出，都会升级为独立可缩放的截图会话窗口；窗口保留冻结画面、选区画布和聊天上下文，支持最小化为多个独立悬浮按钮、点击恢复、吸附回原对话条。创建新截图时会暂时避开已有会话窗口，不会改写或丢失已有会话状态。 / Drag the composer out and then expand history, or expand history before dragging, to upgrade it into a resizable screenshot conversation window. The window preserves the frozen frame, selection canvas and chat context, supports multiple independent floating widgets with restore and docking back to the original composer, and temporarily hides existing conversation windows while a new capture is frozen without changing or losing their state.
